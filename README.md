...
# BrowserStack Examples Appium JUnit 5
...
- How to run the test?
  ...
  Maven:
  ```sh
  mvn clean test -P on-prem -Dsingle.test=LoginDataDrivenTest
  ...
  where, the argument `single.test` or `test-name` can be any testclass implemented this repository.
...
- How to run the test?
  ...
  where, the argument `single.test` or `test-name` can be any testclass implemented in this repository.
